...
class CninfoSpider(scrapy.Spider):
    ...
    # start_urls = [
    #     'http://www.cninfo.com.cn/new/fulltextSearch/full?searchkey=000009%E5%85%AC%E5%8F%B8%E7%AB%A0%E7%A8%8B&sdate=&edate=&isfulltext=false&sortName=nothing&sortType=desc']
    ...
    def parse(self, response):
        ...
        for a in announcements:  # 迭代
            ...
            if re.search('（.*）', announcementtitle):
                ...
                # 以该章程发布的年月日为文件名
                filename = a['adjunctUrl'].split('/')[1] + '.pdf'
                item = PolicyItem(
